/**
 * Scoring must stay finite no matter how hard boons stack — the Balatro "naneinf"
 * endgame. A runaway or buggy boon value must never surface NaN/Infinity in the reveal
 * or silently collapse the score to 0.
 */
...
describe('SafeMath naneinf protection', () => {
    ...
    it('clampScore handles NaN, Infinity, negatives and overflow', () => {
        ...
        expect(S().clampScore(1e400)).toBe(S().MAX_SAFE_INT); // 1e400 overflows to Infinity
    ...
    it('safeMultiply never returns NaN or Infinity', () => {
        ...
        // Overflow clamps to MAX rather than collapsing to 0.
...
describe('ScoringEngine.runPipeline stays finite under runaway boons', () => {
    ...
    // Chance sums all dice: 5×4 = 20 base pips.
    ...
    it('negative favour is held at the 0.1 floor', () => {
        const r = run([boon((d) => { d.favour = -100; return d; })]);
        expect(r.favour).toBe(0.1);
        expect(r.finalScore).toBe(2); // floor(20 × 0.1)
    });
