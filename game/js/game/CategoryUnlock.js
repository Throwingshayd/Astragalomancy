 ...
 * Heureka (six of a kind) and Extra Long Straight (six-run) appear when rolled.
 */
/* exported CategoryUnlock */
/* global SCORING_THRESHOLDS, UNLOCKABLE_SCORE_ROWS, HandEvaluator */
...
const CategoryUnlock = {
    ...
    onRoll(engine) {
        ...
        this.previewHeureka(engine, counts);
        this.previewExtraLongStraight(engine, faces);
    },
    ...
    previewBonusFiveOfAKind(engine, counts) {
        ...
        if (engine.state.bonusYahtzees > 0) {
            this.unlockHighFaces(engine);
            engine.showMessage?.(`Bonus Five of a Kind! (${engine.state.bonusYahtzees} total)`, 3000);
        }
    },
    ...
    previewHeureka(engine, counts) {
        const need = typeof SCORING_THRESHOLDS !== 'undefined' ? SCORING_THRESHOLDS.HEUREKA_REQUIRED : 6;
        if (!Object.values(counts).some((c) => c >= need)) return false;
        return this.unlockOnRoll(engine, 'Heureka', 'Heureka unlocked!');
    },

    previewExtraLongStraight(engine, faces) {
        const need = typeof SCORING_THRESHOLDS !== 'undefined'
            ? SCORING_THRESHOLDS.EXTRA_LONG_STRAIGHT_LENGTH
            : 6;
        if (this.consecutiveRun(faces) < need) return false;
        return this.unlockOnRoll(engine, 'Extra Long Straight', 'Extra Long Straight unlocked!');
    },
